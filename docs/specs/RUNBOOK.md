...
- [ ] **Ten-metre direction test — do this at rehearsal, it is the one that gets skipped.** Stand ten metres from the projector with **both directions running** and check that charge reads differently from V2G **in under two seconds**. The wall is deliberately single-accent: direction is carried by **form**, not hue — solid fill versus hollow ring, pulses travelling inward versus outward. That choice is defensible (direction of travel reads better than hue at distance, the pulse moves the way the energy moves, and a projector distorts colour unpredictably while never distorting motion), but it is **reversible with a test**, and this is the test. NFR-U-2 asks that a viewer tell the two apart *without reading text*, not specifically by colour. **If it does not read in two seconds, the fallback is a white-hot rim on V2G nodes — not a third colour** (`ARCHITECTURE.md` §17)
...
|---|---|---|
| RPC 429s or timeouts, low and flat with load | Normal — ~1–3% transient timeouts at any load is the expected background rate (the old "40–45 req/s knee" is retracted, `REQUIREMENTS.md` §13.4) | Retry (§6 checklist); do not treat this alone as degradation |
| RPC failures **rising** as load rises | Could be a real ceiling this time, or a shared venue IP (monad-facts.md Q2 — per-IP scope is undocumented) | Drop to degraded mode (FR-REL-4 / FR-OPS-3); consider spreading submissions across the other two public endpoints (Ankr ~30 rps avg, Monad Foundation 20 rps — monad-facts.md "Hard constraints" #11) |
