...
| FR-REL-9 | RPC ceiling measured, not assumed | M | T | `node tools/measure-rpc.mjs` (read) and a write-path re-test from **own** wallets at the venue (§6) | **RETRACTED 2026-08-08** (REQUIREMENTS.md §13.4): neither the original read knee nor write ceiling was real — both were transient-timeout noise, not rate-limiting. Current honest statement: "at least 60 tx/s single-wallet, ceiling unknown, ~1–3% transient timeouts at any rate." See §6 | P0 (read) / P2 (write, if re-run before freeze) |
...
**The booth-session path — deleted, not adapted (§16, 2026-08-08).** `API.md` §3's wall-backend-forwards-to-`/v1/readings` design and `ARCHITECTURE.md` ADR-6's `BOOTH_ONCHAIN` flag were two competing proposals for *how* booth ticks might reach the relay. REQUIREMENTS.md §16 didn't pick one — it deleted the premise: the booth app now makes zero chain calls and never reaches the relay at all (FR-SPLIT-1). There is nothing in this test suite to write for the booth path anymore; `test_RelayAcceptsBoothTickSameInterfaceAsSimulated`, `test_BoothForwardingRespectsOnChainFlag`, and `test_BoothSessionRegistersEphemeralKeySilently` (FR-BOOTH-9, itself superseded — no ephemeral key is needed for a client that never signs anything) are all withdrawn along with FR-BOOTH-3/6/9/15/16. The relay's only interaction with the crowd's activity is the one-time `settleRoomAggregate` bridge described in §16.4 (FR-SPLIT-7/8, tested in the §16 table above), which is a normal `settle`-shaped call from the team's own wallet — nothing booth-specific about it at the relay layer.
...
### 6.1 RETRACTED 2026-08-08 — read both this note in full before trusting any number in this section's history

**Neither the read knee (40–45 req/s) nor the write ceiling (10 tx/s) may be quoted as a capacity limit.** REQUIREMENTS.md §13.4 (lines 726–740) retracts both, same day, same section. A re-test from the same wallet returned **25 tx/s: 75/75 clean · 40 tx/s: 109/120 · 60 tx/s: 180/180 clean** — a failure rate that does not rise with load is not a ceiling, it's noise. The original runs' small loss rates (4% at 10 tx/s, 1.1–1.3% at 45–50 req/s) were misread as the onset of rate-limiting; they were transient timeouts (`"The request timed out"`) at a low single-digit rate that occurs at *every* load tested, including loads well below where a real ceiling would bite. Contributing factor: every run used the shared public key `0x…0001`, whose nonce moved from 20 to 89 between runs — other teams were actively transacting from it, and contention was never ruled out.

**The durable finding is not a number — it's that the relay needs retry, which it needed anyway.** Test that property, not a ceiling:
- `test_RelayRetriesOnTransientTimeout` — a submission that times out gets retried automatically, not surfaced as a dropped session or a hard failure
- `test_NoFalseDegradeOnLowRateTimeout` — a 1–3% background timeout rate at nominal load must NOT trip FR-REL-4's degrade-on-429 logic; degrade on a *rising* failure rate, not on noise at any rate

**Current honest capacity statement, quoting REQUIREMENTS.md:740 directly: "at least 60 tx/s single-wallet, ceiling unknown, expect ~1–3% transient timeouts at any rate."** Use this framing everywhere this plan discusses throughput — never a specific ceiling number.

**Consequences for the rest of this test plan, stated plainly (REQUIREMENTS.md:734-738):**
- The zero-margin alarm this section previously raised was false. Ten simulated sessions at 1 Hz is not at any measured ceiling.
- **FR-REL-8's wallet pool is not supported by evidence.** A single wallet sustained 60 tx/s clean. Do not build or test a multi-wallet pool on the strength of this measurement — `test_NoNonceCollisionUnderParallelSubmission` (§5) still tests the *property* (no collisions under whatever pool size is actually built), but this section no longer specifies a pool size or claims the pool is necessary.
- **§16 (the demo/backend split) does not depend on this and is not reopened by it.** It was partly justified by the old numbers, and REQUIREMENTS.md is explicit that part is void — but it stands on reasons that survive the retraction (crowd path independent of public infrastructure, unbounded player count) and it is 15:40+ against an 18:00 freeze. Do not test or write as if §16 is in question.

**What's still genuinely unmeasured, and what would actually settle it (REQUIREMENTS.md:740):** several *own* funded wallets (not the shared `0x…0001` key), from the **venue network**, close to the pitch. `test_WriteCapacityFromOwnWalletsAtVenue` — this is the one measurement in this whole document still worth running before freeze if time allows; everything else quoting a specific tx/s ceiling before this runs is provisional at best.

**`eth_sendRawTransactionSync` is unverified against this endpoint — say so, don't assume it.** Every write measurement so far used viem's standard `sendTransaction` (async `eth_sendRawTransaction`), not the sync variant `ADR-3` specifies. `test_SyncSendMethodAvailable` — an early build task, not an afterthought: call `eth_sendRawTransactionSync` once against `https://testnet-rpc.monad.xyz` and confirm it returns a receipt rather than an unsupported-method error, before the relay's whole submission path is built assuming it works.

**A methodology bug worth testing for, not just reading about (REQUIREMENTS.md:763).** An early run hardcoded `maxFeePerGas` at 60 gwei against a live 102 gwei base fee — every send failed "fee too low," reporting a fabricated sub-2-tx/s ceiling that was a client bug, not a chain limit. `test_FeesReadFromChainNotHardcoded` — inspect the measurement tool: assert `maxFeePerGas` is read live per run, never a hardcoded constant. **Any capacity figure below ~2 tx/s anywhere in this project's history is this bug, not a finding.**

**Booth has no write-path number to worry about at all, retraction or not.** §16 moved booth off-chain entirely — nothing above governs booth. The one remaining on-chain crowd interaction is §16.4's single `settleRoomAggregate` transaction (FR-SPLIT-7/8, §16 table above), one transaction against "at least 60 tx/s" — margin by any reading of the retracted or the original numbers.

### 6.2 What FR-REL-9/W0 means now

The harness this section originally specified (eval-harness E1–E5: ramp rate, measure accepted/refused/latency, decide per-tick-vs-batching against a threshold) is superseded by the fact that the threshold it was built to test against no longer exists. Keep the harness *shape* — it's still how `test_WriteCapacityFromOwnWalletsAtVenue` should be run — but the decision rule is no longer "clear 10 tx/s with margin, else revert to batching." It's simpler now: the design needs ~10 tx/s (AC-5) against a floor of "at least 60 tx/s, ceiling unknown" — there is margin by construction until a real ceiling is found, and per-tick (FR-REL-1) stands unless and until one is.

### 6.3 When to run this

Before freeze if time allows — it is the one honest gap left in this plan's capacity story, not a blocker. `ARCHITECTURE.md`/`DESIGN.md`'s original "first task, before contracts" framing assumed a threshold decision that no longer applies; this is now a nice-to-have confidence check, not a gate.
...
**FR-BOOTH-13's rate cap — 30/s, corrected twice now (see §12).** The requirement went 4,200-score-ceiling → 20 taps/s → **30 taps/s**, and the middle value is itself stale, not just the first: a 20/s cap let a 4-finger human (~20 taps/s) tie a script at the same score, exactly the prize-position tie soft saturation exists to prevent (REQUIREMENTS.md:437). `test_ScoreCapsAtThirtyTapsPerSecond` — feed synthetic tap streams at 35, 50, and 200 taps/s; assert all three produce the identical capped score, and that no plausible human rate (up to ~25/s, five fingers) reaches it. This is the actual anti-cheat mechanism per REQUIREMENTS.md:437: "a script gains nothing... no genuine player is falsely accused."
...
2. **The coverage ledger is stale against the current `REQUIREMENTS.md` on two points, corrected throughout this plan — and `ARCHITECTURE.md` independently found the same thing.** `coverage-ledger.md` describes FR-BOOTH-13 as a "4,200 plausibility ceiling" — the requirement has since moved twice, first to a 20 taps/s hard rate cap, then (2026-08-08, same day) to the current **30 taps/s** (REQUIREMENTS.md:437), specifically because 20/s let a genuine 4-finger player tie a script. Both the ledger's number and this plan's own earlier "20/s" draft were superseded; test against 30/s. The ledger also stops at FR-BOOTH-13; `REQUIREMENTS.md` in its current form additionally defines FR-BOOTH-14, FR-BOOTH-15, and FR-BOOTH-16 (lines 437–439, all Pri `M`), which this plan tests (§3, §10). `ARCHITECTURE.md` §15's own requirement index flags this by name — "Ledger-stale IDs (FR-BOOTH-14/15/16) included" — independent corroboration from a document written concurrently with this one, not by either of us reading the other's work. Per `CLAUDE.md`'s document hierarchy (requirements win on disagreement), this plan tests against the current `REQUIREMENTS.md` text throughout, not the ledger's description.

3. **The `BOOTH_ONCHAIN`-flag-vs-unconditional-forwarding contradiction this plan spent real effort reconciling (ARCHITECTURE.md ADR-6 vs API.md §3) is now moot, superseded by a bigger change.** REQUIREMENTS.md §16 (2026-08-08, "the demo/backend split") didn't resolve that disagreement — it deleted its premise. The booth app makes **zero chain calls**; it was never going to be a question of a flag defaulting true or false, because there is no chain-facing booth code path left at all (FR-SPLIT-1). Recorded here as a reminder of how fast a hackathon's ground truth moves: a carefully-adjudicated three-way document conflict can become irrelevant in the time it takes to write it up. The one real chain interaction the crowd path now has — the single `settleRoomAggregate` bridge transaction (§16.4) — is tested in the §16 table above, most importantly its stall-fallback path (FR-SPLIT-8), which is the part actually likely to matter live.
